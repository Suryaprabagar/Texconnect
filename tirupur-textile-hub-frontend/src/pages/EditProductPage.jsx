...
const EditProductPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const remainingExistingImages = previews.filter(p => p.startsWith('http'));
  ...
  const removeImage = (index) => {
    ...
    // If it's a newly added local file, remove it from productImages
    if (targetPreview.startsWith('blob:')) {
      // Find the index of the file in productImages corresponding to this blob URL
      const localIndex = previews.slice(0, index).filter(p => p.startsWith('blob:')).length;
      setProductImages(productImages.filter((_, i) => i !== localIndex));
    }
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      <div className="mb-10">
        ...
        <p className="text-slate-500 text-lg mt-2 font-medium">Update listing details for your premium garments.</p>
      </div>
